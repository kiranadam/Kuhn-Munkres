...
Hungarian<T>::Hungarian()
{
	...
	m_assignment.resize(m_rows, vector<T>(m_cols,0));
}
...
Hungarian<T>::Hungarian(const vector<vector<T> >& input_matrix, int rows, int cols, MODE mode)
{
  	...
  	m_assignment.resize(rows, vector<T>(cols,0));
  	...
  	else if (mode == HUNGARIAN_MODE_MINIMIZE_COST) 
  	{
    		...
    		cout<<__FUNCTION__<<": unknown mode. Mode was set to HUNGARIAN_MODE_MINIMIZE_COST !\n";
}


// function to print assignment matrix 
template<class T>
...
// function to print cost and assignment matrix 
template<class T>
...
// function to initialized both cost and assigment matrix
template<class T>
...
template<class T>
bool Hungarian<T>::check_solution(const vector<T>& row_dec, const vector<T>& col_inc, const vector<T>& col_vertex)
{
	int k, l, m, n;
...
template<class T>
bool Hungarian<T>::assign_solution(const vector<T>& row_dec,const vector<T>&  col_inc, const vector<T>&  col_vertex)
{
...
bool Hungarian<T>::solve()
{
	...
	vector<T> col_vertex(m), row_vertex(n), unchosen_row(m), parent_row(n),
	...
	m_assignment.assign(m, vector<T>(n, HUNGARIAN_NOT_ASSIGNED));

  	// Begin subtract column minima in order to start with lots of zeroes 12
	if (verbose)
	{
		cout<<"Using heuristic\n";
	}
					...
					if (slack[l])
					{
						...
						if (del<slack[l])
						{
							if (del==0)
							{
								...
								parent_row[l]=k;
								if (verbose)
									cout<<"node "<<t<<": row "<<row_vertex[l]<<"==col "<<l<<"--row "<<k<<"\n";
		...
		for (l=0;l<n;l++)
		{
			...
			if (slack[l])
			{
				...
				if (slack[l]==0)
				{
					...
					if (verbose)
					{
						cout<<"Decreasing uncovered elements by "<<s<<" produces zero at ["<<k<<","<<l<<"]\n";
	...
	if (verbose)
	{
		cout<<"Breakthrough at node "<<q<<" of "<<t<<"!\n";
	}
	while (1)
	{
		j=col_vertex[k];
